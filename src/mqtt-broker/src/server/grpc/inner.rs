...
use crate::inner::services::{
    update_cache_by_req, delete_session_by_req, send_last_will_message_by_req,
};
    ...
    S: StorageAdapter + Sync + Send + 'static + Clone,
{
    ...
    async fn send_last_will_message(
        ...
        let req = request.into_inner();
        send_last_will_message_by_req(&self.cache_manager,&self.client_pool,&self.message_storage_adapter,&req)
        .await
        .map_err(|e| Status::internal(e.to_string()))
        .map(Response::new)
    }
